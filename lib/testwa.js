...
const {
  ...
  util,
  logger
} = require("appium/node_modules/appium-support");
const { retry } = require("appium/node_modules/asyncbox");
const battery = require("./battery");

let log = logger.getLogger("Testwa");
let logData = logger.getLogger("TestWaData");

let temp = require("appium/node_modules/temp"),
...
testwa.initBaseDriver = function(driver) {
  console.log("##########################################");
  console.log("testwa:initBaseDriver");
  console.log("##########################################");
...
testwa.handler = async function(
  ...
  if (osDriver && command) {
    if (command !== "createSession") {
      ...
      if (driver.args.screenshotPath) await mkdirp(driver.args.screenshotPath);
      let platformName = osDriver.caps.platformName.toLowerCase();
...
testwa.beforeExecuteCommand = function(driver, sessionId, command) {
  console.log("##########################################");
  console.log("testwa:beforeExecuteCommand");
  console.log("##########################################");
