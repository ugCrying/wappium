...
function extractProtocol(driver) {
  var sessionId = arguments.length <= 1 || arguments[1] === undefined ? null : arguments[1];
  ...
  return dstDriver ? dstDriver.protocol : PROTOCOLS_MAPPING_CACHE.get(sessionId);
}
...
function checkParams(paramSets, jsonObj, protocol) {
  ...
  try {
    for (
      ...
      if (
        _lodash2['default'].difference(receivedParams, params, optionalParams).length === 0 &&
...
function routeConfiguringFunction(driver) {
  driver.args.screenshotPath = driver.args.screenshotPath || driver.args.screenpath;
  ...
  return function(app) {
    ...
    try {
      for (
        var _iterator3 = _getIterator(_lodash2['default'].toPairs(_routes.METHOD_MAP)), _step3;
        ...
        try {
          for (
            var _iterator4 = _getIterator(_lodash2['default'].toPairs(methods)), _step4;
          ...
          switch ((context$2$0.prev = context$2$0.next)) {
            ...
            case 14:
              ...
              if (spec.command === CREATE_SESSION_COMMAND) {
                ...
                currentProtocol = _basedriverDriver2['default'].determineProtocol.apply(
                  _basedriverDriver2['default'],
                  _toConsumableArray(makeArgs(req.params, jsonObj, spec.payloadParams || {}))
                );
              }
              ...
              // the driver methods
              args = makeArgs(req.params, jsonObj, spec.payloadParams || {}, currentProtocol);
              driverRes = undefined;
              ...
              getLogByProtocol(currentProtocol).debug(
                'Calling ' +
                  ...
                  spec.command +
                  '() with args: ' +
                  _lodash2['default'].truncate(JSON.stringify(args), { length: LOG_OBJ_LENGTH })
              );
              ...
              return _regeneratorRuntime.awrap(
                driver.executeCommand.apply(driver, [spec.command].concat(_toConsumableArray(args)))
              );
            ...
            case 28:
              context$2$0.next = 30;
              return _regeneratorRuntime.awrap(
                driver.execute.apply(driver, [spec.command].concat(_toConsumableArray(args)))
              );
            ...
            case 37:
              ...
              if (spec.command === CREATE_SESSION_COMMAND) {
                ...
                if (currentProtocol === _basedriverDriver2['default'].DRIVER_PROTOCOL.MJSONWP) {
                  driverRes = driverRes[1];
                } else if (currentProtocol === _basedriverDriver2['default'].DRIVER_PROTOCOL.W3C) {
                  driverRes = {
              ...
              // and vice-versa
              if (driverRes) {
                if (currentProtocol === _basedriverDriver2['default'].DRIVER_PROTOCOL.W3C) {
              ...
              if (spec.command === DELETE_SESSION_COMMAND) {
                ...
                getLogByProtocol(currentProtocol).debug('But deleting session, so not returning');
                driverRes = null;
              }
            ...
            case 53:
              ...
              if (currentProtocol === _basedriverDriver2['default'].DRIVER_PROTOCOL.W3C) {
                ...
                currentProtocol === _basedriverDriver2['default'].DRIVER_PROTOCOL.MJSONWP
              ) {
            ...
            case 62:
              ...
              if (_lodash2['default'].isString(httpResBody)) {
                ...
                if (driver && (driver.args.genTool || driver.args.portal)) {
                  testwa.handler(driver, req, httpStatus, httpResBody, spec.command, jsonObj);
                }
    ...
    function doJwpProxy$(context$1$0) {
      while (1)
        switch ((context$1$0.prev = context$1$0.next)) {
          case 0:
            log = getLogByProtocol(extractProtocol(driver, req.params.sessionId));
          ...
          case 4:
            ...
            context$1$0.next = 7;
            return _regeneratorRuntime.awrap(
              driver.executeCommand('proxyReqRes', req, res, req.params.sessionId)
            );
